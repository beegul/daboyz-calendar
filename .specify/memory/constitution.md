...
Every feature and bug fix should be covered by the smallest effective validation needed before merging. Unit tests validate business logic, integration tests validate user flows, and regression tests protect discovered bugs, but coverage should be risk-based rather than volume-based. Core functionality, data integrity, and known regressions must remain protected by automated tests; presentation-only or low-risk changes may rely on targeted manual validation when that is the leaner and more maintainable choice. Retained automated tests must be runnable locally and pass in CI before approvals are granted.
...
- New behavior should include appropriate validation and documentation of expected behavior, with the validation depth scaled to the risk and impact of the change.
...
- Every pull request MUST include a summary of changes, linked validation evidence, and any new UX expectations.
- Reviewers MUST verify that code quality, validation coverage, UX consistency, and performance expectations are met.
